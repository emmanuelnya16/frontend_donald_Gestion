...
export default function SalesView({ user }: SalesViewProps) {
  ...
  const getBoutiqueStats = (boutiqueId: string) => {
    ...
    const totalAmount = allSales.reduce((acc, s) => acc + Number(s.totalPrice), 0);
    const todayAmount = todaySales.reduce((acc, s) => acc + Number(s.totalPrice), 0);
  ...
  if ((loading || productsLoading) && boutiques.length === 0 && allProducts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] space-y-4">
        ...
        <p className="text-slate-500 font-medium">Chargement des données de ventes...</p>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        {user.role === 'ROLE_ADMIN' && (
          <div className="flex flex-col sm:flex-row bg-white p-1 rounded-xl shadow-sm border border-slate-100 gap-1 w-full md:w-auto">
            <button 
